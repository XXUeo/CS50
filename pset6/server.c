...
char* indexes(const char* path)
{
    ...
    int path1 = path_length;
    char* php = "/index.php";
    char* html = "/index.html";
    char* index = malloc(path_length + 12);
    char* index1 = malloc(path1 + 11);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* http = strstr(blank,"HTTP/1.1");
    if(http == NULL)
    {
          error(505);
          return false;
    ...
    char target[blank - slash + 1];
    strncpy(target, slash, blank - slash);
    target[blank - slash] = '\0';
    ...
    else if(strstr(slash,"? ") != NULL)
    { 
       char newTarget[question - target + 1];
       strncpy(newTarget,target,question-target);
       newTarget[question - target] = '\0';
       strcpy(abs_path, newTarget);
       strcpy(query,"");
    ...
    char* divider = strtok(target, "?");
    printf("%s",divider);
    strcpy(abs_path,divider);
    divider = strtok(NULL,"?");
    strcpy(query, divider);
